...
export class WalletComponent implements OnInit {
  ...
  get_order_amount() {
    this.makeapi.getboredercollection(this.uid).subscribe((res) => {
      debugger;
      this.order_details = [];
      res.map((e: any) => {
      ...
      if(this.order_details.length!=0){
        for (let i = 0; i < this.order_details.length; i++) {
          debugger;
          var temp_amount = this.order_details[i].price * this.order_details[i].quantity
          this.user_wallet_amount -= temp_amount
        }
        debugger
        if(this.user_wallet_amount<0){
          this.user_wallet_amount = 0;
          this.userDetail();
        }
        this.userDetail();
